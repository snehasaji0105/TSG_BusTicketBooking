...
export class AddBusComponent implements OnInit {
  ...
  credentialService: any;
   constructor(private addBusService :AddBusService,private formBuilder: FormBuilder, private _router : Router) {
     this.createForm()
    }
 
   ngOnInit(): void {
     this.addBusService.getBus().subscribe(
       (response: any)=>{
   ...
   addBus(){  console.log(this.addBusForm.value)
     ...
     console.log("req",reqObj)
       ...
       this.addBusService.addBus(reqObj).subscribe(
         (response:any)=>{
           console.log(response)
